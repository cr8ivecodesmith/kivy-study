...
class Params:
    ...
    def init(self):
        # This is a scale we've figured out through experimentation.
        w, h = Window.size
        ws = float(w) / 288
        hs = float(h) / 384
...
class Background(Widget):
    ...
    def update(self):
        ...
        self.image_dup.x -= 2 * params.scale
...
class Pipes(Widget):
    ...
    def update(self, dt):
        ...
        if self.add_pipe < 0:
            y = random.randint(
                int(self.y + 50 * params.scale),
                int(self.height - 50 - 3.5 * 24 * params.scale)
            )
...
class Game(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        # Init the bird
        self.bird = Bird(
            pos=(20 * params.scale, self.height / 2)
        )
...
if __name__ == '__main__':
    ...
    # Config.set('kivy', 'dpi', 480)
    # Config.set('graphics', 'metrics_density', 3)
